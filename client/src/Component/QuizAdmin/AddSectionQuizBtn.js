...
import {AddQuizSection} from '../ApiFunctions/AddQuizApi';
import 'react-toastify/dist/ReactToastify.css';
import {toast, ToastContainer} from 'react-toastify';
const AddSectionQuizBtn = ({part, course, section, refreshData}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    let result = AddQuizSection(questions, course.label, section.value, QuizName);
    if (result) {
      setQuestions([]);
      setQuizName("");
      toast.success('QuizAdded', {
        position: 'bottom',
        autoClose: 1500, // Time in milliseconds, set to 0 to disable auto-close
      });
    }
    else {
...
export default AddSectionQuizBtn;
